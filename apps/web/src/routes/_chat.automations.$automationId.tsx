...
function formatRunTimestamp(value: string | null, t: (text: string) => string): string {
  if (!value) return "—";
  ...
  if (dayDelta === 0) return `${t("Today at")} ${time}`;
  if (dayDelta === 1) return `${t("Tomorrow at")} ${time}`;
  if (dayDelta === -1) return `${t("Yesterday at")} ${time}`;
        ...
        >
          ...
          >
            ...
            >
              ...
              <h1 className="truncate font-heading text-sm font-medium">{t("Automations")}</h1>
            </div>
          </header>
            ...
            <div className="flex flex-col gap-6 px-4 py-8">
              ...
              <DetailGroup title={t("Status")}>
                ...
                <DetailRow label={t("Next run")}>
                  {definition.enabled && definition.nextRunAt ? (
                    <StatusValue tone="muted">
                      {formatRunTimestamp(definition.nextRunAt, t)}
                    </StatusValue>
                  ) : (
                    "—"
                  )}
                ...
                <DetailRow label={t("Last ran")}>
                  {lastRun ? (
                    <StatusValue tone="muted">
                      {formatRunTimestamp(lastRun.finishedAt ?? lastRun.startedAt, t)}
                    </StatusValue>
                  ) : (
                    "—"
                  )}
                    ...
                    <div className="space-y-0.5">
                      ...
                      {definition.disabledAt ? (
                        <p className="text-[0.6875rem] text-muted-foreground">
                          {formatRunTimestamp(definition.disabledAt, t)}
                        </p>
                      ) : null}
                    </div>
                    <Button
              ...
              <DetailGroup title={t("Details")}>
                {!ownsItsEnvironment ? (
                  ...
                  <DetailRow label={t("Runs in")}>
                    {t(worktreeModeLabel(definition.worktreeMode))}
                  </DetailRow>
                ) : (
                  <EditRow
                    label={
                ...
                <EditRow label={t("Max iterations")}>
                  <InlineSelect
                    ...
                    options={maxIterationOptions(definition.maxIterations).map((option) => ({
                      ...option,
                      label: t(option.label),
                    }))}
                    disabled={!editable}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1 truncate">
        <span className="text-foreground/90">{t(runStatusLabel(run.status))}</span>
        {resultTitle ? <span className="text-foreground/90"> · {t(resultTitle)}</span> : null}
        <span className="text-muted-foreground"> · {t(runResultSummary(run))}</span>
      </div>
      {triageActionable ? (
      ...
      <span className="shrink-0 tabular-nums text-muted-foreground">
        {formatRelativeTime(run.finishedAt ?? run.startedAt ?? run.scheduledFor, language)}
      </span>
    </div>
  );
}
