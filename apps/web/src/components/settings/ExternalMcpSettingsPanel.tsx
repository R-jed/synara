  ...
  const createMutation = useMutation({
    ...
    onError: (error: unknown) =>
      toastManager.add({
        ...
        description: tError(error, "External MCP setup failed."),
      }),
  });
  ...
  const revokeMutation = useMutation({
    ...
    onError: (error: unknown) =>
      toastManager.add({
        ...
        description: tError(error, "Revocation failed."),
      }),
  });
  ...
  const refreshPairingMutation = useMutation({
    ...
    onError: (error: unknown) =>
      toastManager.add({
        ...
        description: tError(error, "Pairing refresh failed."),
      }),
  });
  ...
  const setupStatus = revoked
    ...
    : integrationExpired
      ...
      : connected
        ? t("Connected")
        : paired
          ? t("Paired — waiting for first use")
          : pairingExpired
            ? t("Pairing code expired")
  ...
  return (
    <div className="space-y-6">
          ...
          >
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-xs font-medium">{t("Read other project tasks")}</div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-xs font-medium">{t("Use the shared local checkout")}</div>
                  <div className="mt-0.5 text-[11px] leading-relaxed text-muted-foreground">
                    {t(
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-xs font-medium">{t("Run without approval prompts")}</div>
                  <div className="mt-0.5 text-[11px] leading-relaxed text-muted-foreground">
                    {t(
      ...
      {setup && setupIntegration && setupPrompt && manualConfiguration && examplePrompt ? (
        <SettingsSection title={`${t("Connect")} ${setupIntegration.name}`}>
          <SettingsRow
            title={
          ...
          <SettingsRow
            ...
            control={
              <Button
                ...
                onClick={() => copyWithToast(setupPrompt, t("Setup prompt copied"), t, tError)}
              >
                {t("Copy setup prompt")}
              </Button>
            }
          ...
          >
            ...
            >
                ...
                <div>
                  <div className="mb-2 flex items-center justify-between gap-3">
                    ...
                    <Button
                      ...
                      disabled={setupUnavailable}
                      onClick={() =>
                        copyWithToast(setup.setupCommand, t("Pairing command copied"), t, tError)
                      }
                    >
                      {t("Copy")}
                    </Button>
              ...
              <div>
                <div className="mb-2 flex items-center justify-between gap-3">
                  <span className="text-xs font-medium">{t("MCP configuration (JSON)")}</span>
                  <Button
                    size="xs"
                    variant="outline"
                    disabled={revoked || integrationExpired}
                    onClick={() =>
                      copyWithToast(manualConfiguration.value, t("Configuration copied"), t, tError)
                    }
                  >
                    {t("Copy")}
                  </Button>
          ...
          <SettingsRow
            ...
            control={
              <Button
                ...
                onClick={() => copyWithToast(examplePrompt, t("Example prompt copied"), t, tError)}
              >
              ...
              <SettingsListRow
                ...
                description={
                  <div className="space-y-1">
                    <div>{status}</div>
                    <div>
                      {t("Projects")}: {describeExternalMcpProjects(integration, t)}
                    </div>
                    <div>
                      {t("Permissions")}:{" "}
