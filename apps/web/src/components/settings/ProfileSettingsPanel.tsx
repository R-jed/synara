...
function ProfileContent({
  ...
  const peakHourLabel = formatPeakHourLabel(stats.activeHours.startHour, language);
  const mostWorkedProjectLabel = formatMostWorkedProjectLabel(stats.mostWorkedProject, language, t);
    ...
    <div className="flex min-w-0 flex-col gap-7">
      ...
      <div className="grid grid-cols-2 divide-x divide-y divide-border/50 overflow-hidden rounded-2xl border border-border/60 sm:grid-cols-3 lg:grid-cols-5 lg:divide-y-0">
        ...
        <StatTile
          ...
          value={
            language === "zh-CN"
              ? `${formatNumber(stats.activity.currentStreakDays)} ${t("days")}`
        ...
        <StatTile
          ...
          value={
            language === "zh-CN"
              ? `${formatNumber(stats.activity.longestStreakDays)} ${t("days")}`
...
function formatHour(hour: number, language: "en" | "zh-CN"): string {
  const normalized = ((hour % 24) + 24) % 24;
  if (language === "zh-CN") return `${normalized.toString().padStart(2, "0")}:00`;
  if (normalized === 0) return "12 AM";
  if (normalized === 12) return "12 PM";
  return normalized < 12 ? `${normalized} AM` : `${normalized - 12} PM`;
}

function formatPeakHourLabel(startHour: number | null, language: "en" | "zh-CN"): string {
  return startHour === null ? "—" : formatHour(startHour, language);
}
...
function formatMostWorkedProjectLabel(
  ...
  if (language === "zh-CN") {
    return `${project.title} · ${formatNumber(project.promptCount)} ${t("prompts")}`;
  }
