              ...
              <section aria-label={t("Saved accounts")} className="pt-3">
                ...
                {snapshot.logins.length === 0 ? (
                  <div className="flex flex-col items-center gap-3 py-6 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">{t("No saved logins.")}</p>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {login.status !== "saved" ? (
                              <p className="mt-1 text-xs text-muted-foreground">
                                {t("Unfinished signup")}
                                {login.status === "expired" ? ` ${t("(expired)")}` : ""}
                              </p>
                            ) : null}
                          </div>
                          <Button
                            variant="ghost"
                            size="icon-sm"
                            aria-label={`${t("Reveal password for")} ${login.username || login.origin}`}
                          ...
                          <Button
                            variant="ghost"
                            size="icon-sm"
                            aria-label={`${t("Delete login for")} ${login.username || login.origin}`}
