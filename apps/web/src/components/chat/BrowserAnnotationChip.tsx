...
export function BrowserAnnotationChip({
  ...
  const removeLabel =
    language === "zh-CN"
      ? `移除浏览器标注 ${annotation.ordinal}`
      : `Remove browser annotation ${annotation.ordinal}`;
  const annotationLabel =
    language === "zh-CN"
      ? `浏览器标注 ${annotation.ordinal}：${label}，${pageLabel}`
      : `Browser annotation ${annotation.ordinal}: ${label}, ${pageLabel}`;
  const trigger =
    variant === "list" ? (
      <span
        className={cn(
