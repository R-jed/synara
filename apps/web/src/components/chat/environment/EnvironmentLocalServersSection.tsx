...
function describeServerCount(count: number, t: (text: string) => string): string {
  if (count === 0) return t("No servers running");
  return `${count} ${t(count === 1 ? "server running" : "servers running")}`;
}
      ...
      <ComposerPickerMenuPopup align="start" side="bottom" className="w-72 min-w-72">
        ...
        {localServersQuery.isLoading ? (
          ...
          <LocalServersPlaceholder
            ...
            subtitle={tError(localServersQuery.error, "The scan failed. Try refreshing.")}
          />
        ) : serverCount === 0 ? (
          <LocalServersPlaceholder
            icon={<GlobeIcon className="size-4" />}
